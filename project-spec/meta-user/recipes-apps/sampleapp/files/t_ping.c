...
PRIVATE FCID_TRANSLATE_TYPE s_fcidTranslateTable[] =
{
   ...
   {YM2L, SM2L},     //2.  chirp freq start
   {YM2U, SM2U},     //3.  chirp freq end
   {YMBN, SMBN},     //4.  ping enable on/off
   {YMRD, SMRD},     //5.  down ping range
   {YMRF, SMRF},     //6.  forward ping range
};
/********************        FUNCTION PROTOTYPES             ******************/
...
PRIVATE void T_PING_vRegisterDatabaseItems(void)
{
   ...
   H_DATABASE_bFetchFloat(SMRF, &s_fFwdPingRange);
   H_DATABASE_bRegisterForData(SMRD, M_PING);
   H_DATABASE_bFetchFloat(SMRD, &s_fDwnPingRange);
   H_DATABASE_bRegisterForData(SMRF, M_PING);
...
PRIVATE void T_PING_vInitDatabaseItems(BOOLEAN bRestore)
{
   if (bRestore)
   {
      ...
      H_DATABASE_bUpdateValueFloat(SMRD, 35.0f);    //default range 30 meters
      H_DATABASE_bUpdateValueFloat(SMRF, 15.0f);    //default range 30 meters
...
PRIVATE void T_PING_vHandleNewData(MESSAGE_NEW_DATA *pMsg)
{
   switch (pMsg->FourCharId)
   {
	 ...
	 break;
      case SMRD:
	 s_fDwnPingRange = pMsg->dbValue.fVal;
	 printf("[%d]%s: SMRD (down range) = %f\n", __LINE__, __FUNCTION__, s_fDwnPingRange);
	 break;
      case SMRF:
	 s_fFwdPingRange = pMsg->dbValue.fVal;
	 printf("[%d]%s: SMRF (forward range) = %f\n", __LINE__, __FUNCTION__, s_fFwdPingRange);
	 break;
      case YBIW:
...
PRIVATE void T_PING_vUpdateDatabaseItem(controlChangeType *controlChange)
{
   ...
   printf("local fcid %x(%c%c%c%c)\n", fcidLocal, (U8)fcidLocal, (U8)(fcidLocal >> 8), (U8)(fcidLocal >> 16), (U8)(fcidLocal >> 24));
